...
import OtherPlayerRocket from "@/components/OtherPlayerRocket";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    newSocket.on("playersUpdate", (players) => {
      setOtherPlayers(players.filter((p: any) => p.id !== newSocket.id));
    });
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-24">
      ...
      {otherPlayers.map((player) => (
        <OtherPlayerRocket key={player.id} player={player} />
      ))}
      <GameInfoOverlay
        info={{
          isLoading,
